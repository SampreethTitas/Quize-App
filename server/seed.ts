...
export async function seedDatabase() {
  try {
    ...
    console.log("Cleared existing data, reseeding with comprehensive data...");
    ...
    const nodeSubjectId = insertedSubjects.find(s => s.name === "Node.js")?.id;
    if (nodeSubjectId) {
      const nodeQuestions = [
        {
    ...
    const algoSubjectId = insertedSubjects.find(s => s.name === "Algorithms")?.id;
    if (algoSubjectId) {
      const algoQuestions = [
        {
    ...
    const systemSubjectId = insertedSubjects.find(s => s.name === "System Design")?.id;
    if (systemSubjectId) {
    ...
    const dbSubjectId = insertedSubjects.find(s => s.name === "Databases")?.id;
    if (dbSubjectId) {
      const dbQuestions = [
        {
