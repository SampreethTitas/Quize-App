const CACHE_NAME = 'prepquiz-pro-v1';
const urlsToCache = [
  '/',
  '/manifest.json',
  '/api/subjects',
  // Add other static assets and API routes that should be cached
];
...
self.addEventListener('sync', (event) => {
  if (event.tag === 'background-sync') {
    event.waitUntil(
      // Sync any pending quiz results or data
...
self.addEventListener('push', (event) => {
  const options = {
    body: 'Time for your daily quiz practice!',
  ...
  event.waitUntil(
    self.registration.showNotification('PrepQuiz Pro', options)
  );
});
